...
var stickers = {
    ...
    ban:"and you think that will stop me, huh?",
    flatearth:"this is true, and you cant change my opinion loser",
    swag:"i am prettier and swagger than u lol",
    sans:"frick you!",
    flip:"frick you!",
    topjej:"toppest jej",
    high:"like ur mom",
    sex:"wow. just wow. you wanted henpi and you go for bonzi world instead. wow.",
    cyan:"nevermind",
    no:"no! get out of here and never come back!",
    bye:"goodbye noob",
    kiddie:"you can only go to bonzi.world for that, sorry, kiddie.",
}
...
*/	
    ...
    "2018":function(text){
        this.room.emit('talk',{
            text:`This generation sucks! Adolescents are filled with pornographic obsessions. Since 2018, i fucking hate people like them nowadays. They think they're so funny with their 'funny' hentai profile pictures, and pictures like sonic using a hentai face. It's disgusting, I hate it. But this is cheese world. And every single cheese is aloud to do as they wish! Just kidding, now fuck off.`,
            guid:this.guid
        })
    },
    "behh":function(text){
    ...
    "name": function() {
		...
		if (!Ban.isIn(this.getIp())) {
			...
			if (argsString.includes("Foxy")) {
				argsString = "HEY EVERYONE LOOK AT ME I'M A FAGGY FAGGY FAG FAG POOPY PEEPEE LMAO";
			}
...
class User {
    ...
    talk(data) {
        ...
        if (typeof data != 'object') { // Crash fix (issue #9)
            data = {
                text: "HEY EVERYONE LOOK AT ME I'M TRYING TO KILL CHEESE LORD LMAO"
            };
        }
...
var commands = {
	...
	rant(txt){
        return 'u!help, x!help and other js from inspect element bots or fake bots are gay. they are not real bots. b!help and cheesehelp are cool and they are pretty epic.'
	},
	rant2(txt){
        return 'haha nice goanimate joke hahaha frick you'
	},
	grounded(txt){
        return 'haha nice goanimate joke hahaha frick you'
    },
	...
	default(txt){
        clientsocket.emit('command',{list:["name","CheeseWORLD | +help  "]})
	},
    uglyify(txt){
        return "[["+txt+"]]"
    },
    ...
    clickbait(txt){
        return (["omg!",':O','whaat?','wtf?!'][Math.floor(Math.random()*4)]+' '+txt+' '+["(gone wrong)",'(gone sexual)','(not clickbait!)','(cops called)'][Math.floor(Math.random()*4)]+'\u{1F631}'.repeat(Math.ceil(Math.random()*5))).toUpperCase()
    } 
}
